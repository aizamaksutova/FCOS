...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        self.conv1x1_m3 = nn.Conv2d(dummy_out_shapes[0][1][1], self.out_channels, kernel_size=(1, 1), stride=1, padding=0)
        self.conv1x1_m4 = nn.Conv2d(dummy_out_shapes[1][1][1], self.out_channels, kernel_size=(1, 1), stride=1, padding=0)
        self.conv1x1_m5 = nn.Conv2d(dummy_out_shapes[2][1][1], self.out_channels, kernel_size=(1, 1), stride=1, padding=0)
        self.conv3x3_p3 = nn.Conv2d(self.out_channels,self.out_channels, kernel_size=(3, 3),stride=1,padding=1) 
        self.conv3x3_p4 = nn.Conv2d(self.out_channels,self.out_channels, kernel_size=(3, 3), stride=1, padding=1)
    ...
    def forward(self, images: torch.Tensor):
        ...
        m5 = self.fpn_params["m5"](c5)
        fpn_feats["p5"] = m5

        m5_output = F.interpolate(m5, scale_factor=2)
        m4 = self.fpn_params["m4"](c4) + m5_output
        fpn_feats["p4"] = self.fpn_params["p4"](m4)

        m4_output = F.interpolate(m4, scale_factor=2)
        m3 = self.fpn_params["m3"](c3) + m4_output
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        ######################################################################
        # Replace "pass" statement with your code
        x = stride * torch.arange(0.5, feat_shape[3] + 0.5, step=1, dtype=dtype, device=device)
        y = stride * torch.arange(0.5,feat_shape[2]+0.5,step=1,dtype=dtype,device=device)
        (x_dots, y_dots) = torch.meshgrid(x,y,indexing='xy')
        x_dots = x_dots.unsqueeze(dim=-1)
        y_dots = y_dots.unsqueeze(dim=-1)
        prom = torch.cat((x_dots,y_dots),dim=2)
        location_coords[level_name] = prom.view(feat_shape[3]*feat_shape[2],2)
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(stem_channels)):
          ...
          conv_layer_cls = nn.Conv2d(in_ch, stem_channels[i],kernel_size=(3, 3),stride=1,padding=1)
          torch.nn.init.normal_(conv_layer_cls.weight, 0, 0.01)
          torch.nn.init.constant_(conv_layer_cls.bias, 0)
          

          conv_layer_box = nn.Conv2d(in_ch, stem_channels[i],kernel_size=(3, 3),stride=1,padding=1)
          torch.nn.init.normal_(conv_layer_box.weight, 0, 0.01)
          torch.nn.init.constant_(conv_layer_box.bias, 0)


          stem_cls.append(conv_layer_cls)
          stem_cls.append(relu)
          stem_box.append(conv_layer_box)
        ...
        self.pred_cls = nn.Conv2d(stem_channels[-1], num_classes, kernel_size=(3, 3),stride=1,padding=1)  # Class conv
        torch.nn.init.normal_(self.pred_cls.weight, 0, 0.01)
        torch.nn.init.constant_(self.pred_cls.bias, 0)

        self.pred_box = nn.Conv2d(stem_channels[-1], 4, kernel_size=(3, 3),stride=1,padding=1) # Box regression conv
        torch.nn.init.normal_(self.pred_box.weight, 0, 0.01)
        torch.nn.init.constant_(self.pred_box.bias, 0)

        self.pred_ctr = nn.Conv2d(stem_channels[-1], 1, kernel_size=(3, 3),stride=1,padding=1)  # Centersness conv
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for layer_name in layers:
            fpn = feats_per_fpn_level[layer_name]

            stem_cls = self.stem_cls(fpn)
            pred_cls = self.pred_cls(stem_cls).permute(0, 2, 3, 1).flatten(1, 2)
            class_logits[layer_name] = pred_cls


            stem_box = self.stem_box(fpn)
            pred_box = self.pred_box(stem_box).permute(0, 2, 3, 1).flatten(1, 2)
            boxreg_deltas[layer_name] = pred_box

            centereness= self.pred_ctr(stem_box)
            centerness_logits[layer_name] = centereness.permute(0, 2, 3, 1).flatten(1, 2)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_get_deltas_from_locations(
    ...
    for i in range(4):
        if i % 2 == 0:
            deltas[:, i] = (locations[:, i % 2] - gt_boxes[:, i]) * (stride ** -1)
        else:
            deltas[:, i] = (gt_boxes[:, i] - locations[:, i % 2]) * (stride ** -1)
...
def fcos_apply_deltas_to_locations(
    ...
    output_boxes = torch.empty_like(deltas).to(device=deltas.device)
    ...
    for i in range(4):
        ...
        output_boxes[:, i] = locations[:, axis_index] + sign * stride * deltas[:, i]
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    width = deltas[:, 0] + deltas[:, 2]
    height = deltas[:, 1] + deltas[:, 3]

    centerness = 2 * (torch.min(width, height) / torch.max(width, height)).sqrt() - 1
